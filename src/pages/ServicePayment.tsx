...
const ServicePayment = ({
  ...
  const [dataTab, setDataTab] = useState<
    "HOT DEALS" | "Daily" | "Weekly" | "Monthly" | "Other"
  >("HOT DEALS");
  ...
  const loadBillers = async () => {
    ...
    try {
      ...
      // Electricity must show only the 11 Nigerian DISCos.
      // Aba Electric is intentionally excluded.
      const electricityDiscoNames = new Set([
        "ikeja electric",
        "eko electric",
        "abuja electric",
        "kano electric",
      ...
      const electricityDiscoCodes = new Set([
        "01", "02", "03", "04", "05", "06",
        "07", "08", "09", "10", "11",
      ]);

      const visibleBillers =
        serviceType === "electricity"
          ? loadedBillers.filter((biller: Biller) => {
              const code = cleanString(biller.biller_code);
              const name = cleanString(
                biller.name ?? biller.short_name
              ).toLowerCase();

              return (
                electricityDiscoCodes.has(code) ||
                electricityDiscoNames.has(name)
              );
            })
          : loadedBillers;

      setBillers(visibleBillers);

      if (!visibleBillers.length) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50 pb-8">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-6">
        ...
        {showPinPrompt ? (
          <div className="bg-white dark:bg-white dark:text-black rounded-2xl shadow-sm border p-5 sm:p-6 dark:[&_p]:text-black dark:[&_span]:text-black dark:[&_h2]:text-black dark:[&_h3]:text-black dark:[&_label]:text-black">
          ...
          <div className="bg-white dark:bg-white dark:text-black rounded-2xl shadow-sm border p-5 sm:p-6 dark:[&_p]:text-black dark:[&_span]:text-black dark:[&_h2]:text-black dark:[&_h3]:text-black dark:[&_label]:text-black">
            ...
            <div className="space-y-2 mb-5">
              ...
              <select
                ...
                className="w-full h-11 rounded-md border bg-background px-3 text-sm text-gray-900 dark:text-black"
              >
            ...
            {isData ? (
              <div className="space-y-4 mb-5">
                ...
                <div className="grid grid-cols-5 gap-1.5 sm:gap-2">
                  {([
                    ...
                    <button
                      ...
                      className={[
                        "rounded-lg border px-2 py-2 text-[10px] sm:text-xs font-semibold transition-all",
                        dataTab === tab
                    ...
                    >
                      {tab === "HOT DEALS" && (
                        <Flame className="inline-block h-3 w-3 sm:h-3.5 sm:w-3.5 mr-0.5" />
                      )}
                      {tab}
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-4 gap-2 sm:gap-2.5">
                  {(dataTab === "HOT DEALS"
                    ? items.filter((item) => isHotDeal(item))
                    : dataGroups[dataTab]
                  ).map((item, index) => {
                    ...
                    const selected = selectedItemCode === code;
                    const hotDeal =
                      dataTab === "HOT DEALS" && isHotDeal(item);
                    ...
                    return (
                      <button
                        ...
                        disabled={processingPayment || verifyingPin}
                        className={[
                          "relative min-w-0 text-left rounded-xl border p-2.5 sm:p-3 transition-all",
                      ...
                      >
                        {hotDeal && (
                          <span className="absolute -top-2 -right-1.5 inline-flex items-center gap-0.5 rounded-full bg-orange-500 px-1.5 py-0.5 text-[8px] font-extrabold uppercase leading-none text-white shadow-sm">
                        ...
                        <p className="text-sm font-medium text-gray-900 line-clamp-2">
                          {item.name ?? item.short_name ?? "Data Plan"}
                        </p>
                        ...
                        <p className="text-[10px] text-gray-500 mt-1 line-clamp-1">
                          {item.validity ?? item.duration ?? getDataGroup(item)}
                        </p>
                      </button>
                    );
                  })}
