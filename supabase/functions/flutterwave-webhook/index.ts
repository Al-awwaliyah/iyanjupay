import {
   ...
   async function handleTransferWebhook(
     ...
     if (
       ...
       const kycAlreadyCompleted = currentMetadata?.kyc_limit_completed === true;
       if (!kycAlreadyCompleted) {
         const { data: kycCompletion, error: kycCompletionError } = await supabase.rpc("complete_kyc_daily_transfer", { _user_id: transaction.user_id, _amount: providerTransferAmount });
         if (kycCompletionError || !kycCompletion?.success) {
           console.error("KYC completion failed:", kycCompletionError || kycCompletion);
           return jsonResponse({ success: false, status: "pending", error: "Transfer succeeded at Flutterwave, but daily transfer-limit finalization is pending. The webhook will retry.", transaction_id: transaction.id, transfer_id: transferId }, 503);
         }
         currentMetadata.kyc_limit_completed = true;
         currentMetadata.kyc_completed_amount = providerTransferAmount;
         currentMetadata.kyc_completed_at = new Date().toISOString();
       }

       const electronicFeeDue = providerTransferAmount > FLUTTERWAVE_ELECTRONIC_FEE_THRESHOLD ? ELECTRONIC_FEE : 0;
       let electronicFeeCharged = currentMetadata?.electronic_fee_charged === true;
       let electronicFeePending = currentMetadata?.electronic_fee_pending === true;
       let electronicFeeError = currentMetadata?.electronic_fee_error ?? null;
       if (electronicFeeDue > 0 && !electronicFeeCharged) {
         const feeKey = `ELECTRONIC_FEE_${transaction.id}`;
         const { data: feeResult, error: feeError } = await supabase.rpc("wallet_operation", { _user_id: transaction.user_id, _operation: "DEBIT", _amount: electronicFeeDue, _description: `Electronic transfer fee for Flutterwave transfer of ₦${providerTransferAmount.toLocaleString()}`, _idempotency_key: feeKey, _reference: `ELECTRONIC_FEE_${transaction.id}`, _provider: "flutterwave", _category: "electronic_transfer_fee", _metadata: { original_transaction_id: transaction.id, flutterwave_transfer_id: transferId, transfer_amount: providerTransferAmount, electronic_fee: electronicFeeDue, currency: "NGN" } });
         if (feeError || !feeResult) { electronicFeePending = true; electronicFeeError = feeError?.message || "Electronic transfer fee debit failed."; console.error("Electronic fee debit failed:", feeError || feeResult); } else { electronicFeeCharged = true; electronicFeePending = false; electronicFeeError = null; }
       }
       const totalElectronicFee = electronicFeeCharged ? electronicFeeDue : 0;
       ...
       const {
         ...
         .update({
           ...
           metadata: {
             ...
             kyc_completed_amount: providerTransferAmount,
             electronic_fee_due: electronicFeeDue,
             ...
             electronic_fee_amount_charged: totalElectronicFee,
           },
         })
         .eq(
           "id",
       ...
       return jsonResponse({
         ...
         total_charged:
           transactionTotalCharged + totalElectronicFee,
     ...
     if (
       ...
       /*
       * ========================================================
       * RELEASE KYC DAILY TRANSFER RESERVATION
       * ========================================================
       */

      if (transactionMetadata?.kyc_limit_completed !== true) {
        const { data: kycRelease, error: kycReleaseError } = await supabase.rpc("release_kyc_daily_transfer", { _user_id: transaction.user_id, _amount: providerTransferAmount });
        if (kycReleaseError || !kycRelease?.success) console.error("KYC reservation release failed:", kycReleaseError || kycRelease);
        else { transactionMetadata.kyc_limit_released = true; transactionMetadata.kyc_released_amount = providerTransferAmount; transactionMetadata.kyc_released_at = new Date().toISOString(); }
      }
