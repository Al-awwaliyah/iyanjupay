...
export default function SecuritySettingsPage({ onBack }: { onBack: () => void }) {
  ...
  const [timeout, setTimeoutValue] = useState(() => localStorage.getItem(APP_LOCK_TIMEOUT_KEY) ?? "0");
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-6">
      <div className="mx-auto max-w-3xl space-y-5">
        ...
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><LockKeyhole className="h-5 w-5" />App Lock</CardTitle><CardDescription>Lock the app after inactivity or backgrounding. Unlock with your enabled biometric authentication.</CardDescription></CardHeader><CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-4"><div className="space-y-1"><Label>Lock after</Label><p className="text-xs text-slate-500">Choose how quickly IyanjuPay locks after you leave or background the app.</p></div><Select value={timeout} onValueChange={changeTimeout} disabled={!appLock}><SelectTrigger className="w-44"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="0">Immediately</SelectItem><SelectItem value="60">1 minute</SelectItem><SelectItem value="300">5 minutes</SelectItem><SelectItem value="900">15 minutes</SelectItem><SelectItem value="1800">30 minutes</SelectItem></SelectContent></Select></div>
