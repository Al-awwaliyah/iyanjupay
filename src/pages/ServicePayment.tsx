...
function billerCodeOf(biller: Biller): string {
  return cleanString(
    biller.biller_code ??
      biller.code ??
      biller.id ??
...
function itemCodeOf(item: BillItem): string {
  return cleanString(
    item.item_code ??
      item.code ??
      item.id ??
...
function itemNetworkOf(item: BillItem): string {
  return cleanString(
    item.networkCode ??
      item.network_code ??
      item.network ??
...
const ServicePayment = ({
  ...
  const selectedItem = useMemo(
    () =>
      items.find(
        (item) =>
          itemCodeOf(item) === selectedItemCode
      ) ?? null,
    [
      items,
      selectedItemCode,
  ...
  const loadBillers = async () => {
    ...
    try {
      const { data, error: functionError } =
        await supabase.functions.invoke("peyflex-services", {
          body: {
            action: "catalog",
            service: serviceType,
            country: "NG",
      ...
      const loadedBillers = (Array.isArray(data?.billers)
        ...
        : Array.isArray(data?.networks)
          ...
          : Array.isArray(data?.providers)
            ...
            : []).map((biller: Biller) => ({
              ...biller,
              biller_code: billerCodeOf(biller),
            }));
  ...
  const loadItems = async (
    ...
    try {
      ...
      const normalizedItems = loadedItems
        .map((item: BillItem) => ({
          ...item,
          item_code: itemCodeOf(item),
          plan_type: item.plan_type ?? (isHotDeal(item) ? "SME" : "REGULAR"),
          is_hot_deal: item.is_hot_deal === true || isHotDeal(item),
        }))
        .filter((item: BillItem) => {
          if (isEducation) {
            const provider = cleanString(item.providerCode ?? item.provider_code ?? item.biller_code);
            return !provider || provider === cleanBillerCode;
          }
          if (isRechargeCard) {
            const network = itemNetworkOf(item);
            return !network || network.toLowerCase() === cleanBillerCode.toLowerCase();
          }
          return true;
        });
  ...
  const handleItemChange = (
    ...
    const item = items.find((entry) => itemCodeOf(entry) === value);
    if (!isAmountBased && item) {
  ...
  const validateForm =
    (): boolean => {
      ...
      if (!isAmountBased && !selectedItemCode) {
        toast({
          ...
          <div className="bg-white rounded-2xl shadow-sm border p-5 sm:p-6">
            ...
            <Button
              ...
              disabled={
                ...
                verifyingPin ||
                !selectedBillerCode ||
                (!isAmountBased && !selectedItemCode) ||
                !customer.trim() ||
                !amount
              }
