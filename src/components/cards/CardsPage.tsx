...
const CardsPage = ({ onBack }: CardsPageProps) => {
  ...
  const callCards = useCallback(
    async (payload: Record<string, unknown>) => {
      const { data, error } =
        await supabase.functions.invoke("flutterwave-card", {
          body: payload,
        });
  ...
  const handleFund = async (
    ...
    const amount = Number(
      fundAmounts[card.provider_card_id] ?? ""
    );
    ...
    if (
      ...
      toast({
        title: "Invalid amount",
        description:
          "Enter the amount you want to add to this card.",
        variant: "destructive",
      });
  ...
  const handleLifecycle = async (
    ...
    if (
      action === "terminate" &&
      !window.confirm(
        "Terminate this card permanently?"
      )
    ) {
      return;
    }
        ...
        <Card className="mb-6">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-gray-500">
              Cards are issued instantly and
              funded from your IyanjuPay wallet.
              BVN verification is required before
              your first card.
            </p>

            <div className="space-y-2">
              <Label htmlFor="issueAmount">
            ...
            {loadingCards ? (
              ...
              cards.map((card) => (
                ...
                >
                  ...
                  <div className="rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-4">
