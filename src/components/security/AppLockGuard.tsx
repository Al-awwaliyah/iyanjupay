...
function readTimeout() { return Math.max(30, Number(localStorage.getItem(TIMEOUT_KEY) || 300)); }
...
export default function AppLockGuard({ children }: { children: React.ReactNode }) {
  ...
  const unlock = async () => {
    ...
    setError("");
    try {
      if (!("PublicKeyCredential" in window)) throw new Error("This device does not support passkeys.");
      const { error: authError } = await (supabase.auth as any).signInWithPasskey();
      if (authError) throw authError;
      setLocked(false);
    } catch (error: any) {
      setError(error?.message || "Biometric/passkey verification failed.");
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950 p-6 text-white">
      <div className="w-full max-w-sm rounded-3xl border border-white/10 bg-slate-900 p-7 text-center shadow-2xl">
        ...
        <p className="mt-2 text-sm text-slate-400">Unlock with your registered device passkey, biometrics, Face ID, Touch ID, or device PIN.</p>
