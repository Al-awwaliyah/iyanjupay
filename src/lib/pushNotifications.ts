...
export async function restoreWebPushRegistration() {
  ...
  try {
    ...
    if (Capacitor.getPlatform() === "android") {
      await PushNotifications.createChannel({
        id: "iyanjupay_notifications",
        name: "IyanjuPay notifications",
        description: "IyanjuPay transaction and account notifications",
        importance: 5,
        visibility: 1,
        sound: "default",
      }).catch(() => undefined);
...
export async function restorePushRegistration() {
  const enabled = localStorage.getItem("iyanjupay-push-enabled") === "true";
